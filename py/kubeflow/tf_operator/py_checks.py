...
def run_lint(args):
  ...
  for root, dirs, files in os.walk(os.path.abspath(args.src_dir), topdown=True):
    exclude = any(root.startswith(e) for e in full_dir_excludes)
    if exclude:
      continue
    ...
    for pat in includes:
      for f in fnmatch.filter(files, pat):
        ...
        try:
          util.run(["pylint", f"--rcfile={rc_file}", full_path], cwd=args.src_dir)
...
def run_tests(args):
  ...
  env["PYTHONPATH"] = f"{args.src_dir}:" + os.path.join(
      args.src_dir, "kubeflow_testing", "py")
